	...
	{	
		...
		try
		{
			...
			if (i_irdata_type == IR_DATA_AIRCON_ALL_NAME) {			// 获取所有的空调名字
				...
				if(!bRet)
				{
					LogWriter.WriteDebugLog(LogWriter.SELF,String.format("[AirCon]Aircon Name Save fail(%s): %s", strModuleId, strAirConName));
					dbMgr.Rollback();
					...
					LogWriter.WriteDebugLog(LogWriter.SELF,String.format("[AirCon]Aircon Name Save OK(%s): %s", strModuleId, strAirConName));
				}
				...
				if (strAirConName.isEmpty() == true) {
					LogWriter.WriteDebugLog(LogWriter.SELF,String.format("[AirCon]Aircon Name Load is null (%s): %s", strModuleId, strAirConName));	
				} else {
					LogWriter.WriteDebugLog(LogWriter.SELF,String.format("[AirCon]Aircon Name Load OK (%s): %s", strModuleId, strAirConName));
				}
